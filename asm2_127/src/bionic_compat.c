...
/* Android ARM32 struct statfs uses 64-bit block and file counters.  The host
 * glibc ARM32 layout is different, so translate field by field instead of
 * writing a host struct into guest memory. */
struct asm2_statfs {
  uint32_t f_type;
  uint32_t f_bsize;
  uint64_t f_blocks;
